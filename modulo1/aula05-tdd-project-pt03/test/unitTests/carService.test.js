...
describe('CarService Suit Tests', () => {
  ...
  it('given a CarCategory customer and numberOfDays it should calculate final amount in R$', async() => {
    ...
    //para não depender da classe tax
    sandbox.stub(
      carService,
      "taxesBaseOnAge"
  ...
  it('give a customer and a CarCategory it should return a transaction receipt', async() => {
    const car = mocks.validCar
    const carCategory = {
    ...
    const expected = new Transaction({
      customer, car, dueDate, amount: expectedAmount,
    })
    expect(result).to.be.deep.equal(expected)
  })
